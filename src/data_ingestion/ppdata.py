...
from src.data_ingestion.ingestion_utils import (
    ...
    log_rejected_record, update_ingestion_status, parse_time
)
...
def process_ppData_file(xml_file, xsd_schema_path, conn, cursor):
    ...
    try:
            ...
            try:
                ...
                if course_code and course_code != 'Unknown':
                    ...
                    if mapped_course_cd and len(mapped_course_cd) == 3:
                        ...
                        logging.error(f"Course code '{course_code}' not found in mapping dictionary or invalid. Defaulting to 'UNK'.")
                        course_cd = 'UNK'
                else:
                    logging.error("Course code not found in XML or is 'Unknown'. Defaulting to 'UNK'.")
                    course_cd = 'UNK'

                race_date = parse_date(get_text(race.find('race_date'), '1970-01-01'))
                race_number = safe_int(get_text(race.find('race'), '0'))
                post_time = parse_time(get_text(race.find('post_time'))) or datetime(1970, 1, 1).time()

                for horse in race.findall('horsedata'):
                    try:
                        saddle_cloth_number = get_text(horse.find('program'), '0')
                        ...
                        for ppdata_data in horse.findall('ppdata'):
                            try:
                                ...
                                # Extract pp_course_cd
                                trackcode = get_text(ppdata_data.find('trackcode'), 'Unknown')
                                if trackcode and trackcode != 'Unknown':
                                    mapped_pp_course_cd = eqb_tpd_codes_to_course_cd.get(trackcode)
                                    if mapped_pp_course_cd and len(mapped_pp_course_cd) == 3:
                                        pp_course_cd = mapped_pp_course_cd
                                    else:
                                        logging.error(f"Trackcode '{trackcode}' not found in mapping dictionary or invalid. Defaulting to 'UNK'.")
                                        pp_course_cd = 'UNK'
                                else:
                                    logging.error("Trackcode not found in ppdata or is 'Unknown'. Defaulting to 'UNK'.")
                                    pp_course_cd = 'UNK'
                                ...
                                # SQL insert query for the ppdata table
                                insert_query = """
                                ...
                                try:
                                    ...
                                    has_rejections = True
                                    logging.error(f"Error processing race {race_number}, saddle cloth {saddle_cloth_number}: {ppData_error}")
                                    conn.rollback()
                                    ...
                                    rejected_record = {
                                        ...
                                        # Include other relevant fields for logging
                                        "pp_course_cd": pp_course_cd,
                                        "pp_race_date": pp_race_date.isoformat() if pp_race_date else None,
                                ...
                                has_rejections = True
                                conn.rollback()
                                logging.error(f"Error processing ppdata for horse {saddle_cloth_number} in race {race_number}: {e}")
                                rejected_record = {
                                    "course_cd": course_cd,
                                    "race_date": race_date.isoformat() if race_date else None,
                                    "post_time": post_time.isoformat() if post_time else None,
                        ...
                        has_rejections = True
                        conn.rollback()
                        logging.error(f"Error processing horse data in race {race_number}: {e}")
                        rejected_record = {
                            "course_cd": course_cd,
                            "race_date": race_date.isoformat() if race_date else None,
                            "post_time": post_time.isoformat() if post_time else None,
        ...
        logging.error(f"Critical error processing ppData file {xml_file}: {e}")
        conn.rollback()
        update_ingestion_status(conn, xml_file, "error", "ppData")
