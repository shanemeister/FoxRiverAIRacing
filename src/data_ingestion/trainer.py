        ...
        for race in root.findall('racedata'):
            ...
            for horse in race.findall('horsedata'):
                try:
                    ...
                    logging.error(f"Error processing trainer data for horse {axciskey} in file {xml_file}: {e}")
                    conn.rollback()  # Rollback the transaction for this horse
                    continue  # Skip to the next horse

        # Commit the transaction after all trainer data has been processed
        conn.commit()
        
    except Exception as e:
        # Handle exceptions and rollback in case of error
        logging.error(f"Error processing trainer data in file {xml_file}: {e}")
