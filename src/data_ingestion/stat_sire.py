                        ...
                        if stats_data is not None:
                            for stat in stats_data.findall('stat'):
                                try:
                                    ...
                                    logging.error(f"Error processing stat '{stat_type}' for sire '{sirename}' of horse {axciskey} in file {xml_file}: {e}")
                                    conn.rollback()  # Rollback the transaction for this stat record
                                    continue  # Skip to the next stat
                        else:
                            logging.warning(f"No stats_data found for sire '{sirename}' of horse {axciskey} in file {xml_file}.")
                    else:
                        logging.warning(f"No sire data found for horse {axciskey} in file {xml_file}.")
                except Exception as e:
                    logging.error(f"Error processing stat_sire data for horse {axciskey} in file {xml_file}: {e}")
                    conn.rollback()  # Rollback the transaction for this horse
                    continue  # Skip to the next horse

        # Commit the transaction after all stat_sire data has been processed
        conn.commit()
 
    except Exception as e:
        # Handle exceptions and rollback in case of error
        logging.error(f"Error processing stat_sire data in file {xml_file}: {e}")
