...
from src.train_equibase.equibase_load_training_data import load_base_training_data
...
from src.train_equibase.equibase_build_cat_model import build_catboost_model
...
def setup_logging():
    ...
    try:
        ...
        log_file = os.path.join(log_dir, 'Equibase_Pred_load.log')
...
def main():
    ...
    try:
        ...
        ###################################################
        # 2) Prep data for Training or Predictions
        ###################################################
        # Load the Parquet file into a Pandas DataFrame.
        train_df = pd.read_parquet("/home/exx/myCode/horse-racing/FoxRiverAIRacing/data/parquet/train_df", engine="pyarrow")
        ...
        scored_sdf = build_catboost_model(spark, train_df, jdbc_url, jdbc_properties) #spark, parquet_dir, speed_figure) # model_filename)
