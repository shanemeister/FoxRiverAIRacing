...
def load_base_training_data(spark, jdbc_url, jdbc_properties, parquet_dir):
    ...
    train_df.select("distance", "dist_unit", "distance_meters").show(10, truncate=False)
    
    # Drop the old columns
    train_df = train_df.drop("distance", "dist_unit")
    ...
    #     mean_value = train_df.select(F_mean(col(col_name))).first()[0]
    ...
    # train_df = train_df.fillna({col_name: 0 for col_name in zero_fill_cols})
    ...
    # train_df = train_df.fillna({col_name: -1 for col_name in negative_fill_cols})

    # # Impute categorical columns with the "MISSING"
    # cat_cols = ["surface", "trk_cond", "weather", "race_type", "stk_clm_md"]
    # for col_name in cat_cols:
    #     train_df = train_df.fillna({col_name: "MISSING"})
    
    # filtered_df = train_df.filter(col("days_off").isNull()).select("layoff_cat", "days_off")
