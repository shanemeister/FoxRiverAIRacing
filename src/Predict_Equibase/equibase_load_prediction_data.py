...
from src.Predict_Equibase.equibase_predict_sql_queries import sql_queries
...
def load_prediction_data_final(spark, jdbc_url, jdbc_properties, parquet_dir):
    """
    Load Parquet file used for prediction data from PostgreSQL.
    """
    prediction_df = None
    queries = sql_queries()
    for name, query in queries.items():
        if name == "prediction_data":
            logging.info("Query prediction_data located and loading from PostgreSQL...")
    ...
    if prediction_df is None:
        logging.error("No prediction_data query found; prediction_df is not defined.")
    ...
    duplicates = (
        prediction_df.groupBy(*primary_keys)
    ...
    # prediction_df = impute_with_race_mean(prediction_df, cols_to_impute)
    ...
    # prediction_df = fill_missing_sectionals(prediction_df)
    ...
    prediction_df = prediction_df.fillna(categorical_defaults)
    # Impute med with NONE
    prediction_df = prediction_df.withColumn("med", when(col("med") == "", "NONE").otherwise(col("med")))
    # Impute turf_mud_mark with MISSING
    prediction_df = prediction_df.withColumn("turf_mud_mark",when(col("turf_mud_mark") == "", "MISSING").otherwise(col("turf_mud_mark")))

    # Impute horse_itm_percentage with 0 when it is null
    prediction_df = prediction_df.withColumn("horse_itm_percentage", when(col("horse_itm_percentage").isNull(), 0).otherwise(col("horse_itm_percentage")))
    
    # Set empty values to 0 for prev_speed and count_workouts_3
    prediction_df = prediction_df.withColumn("prev_speed", when(col("prev_speed").isNull(), 0).otherwise(col("prev_speed")))
    prediction_df = prediction_df.withColumn("count_workouts_3", when(col("count_workouts_3").isNull(), 0).otherwise(col("count_workouts_3")))
    ...
    # Example usage:
    prediction_df = fix_outliers(prediction_df)

    # columns you consider critical
    # prediction_df = prediction_df.na.drop(subset=critical_cols)
    ...
    for c in cols_to_fill:
        prediction_df = prediction_df.withColumn(
            c,
            F.last(prediction_df[c], ignorenulls=True).over(win)
        )  
    ...
    logging.info(f"5. Just after impute_performance_columns:  Number of rows prediction data: {prediction_count}")
