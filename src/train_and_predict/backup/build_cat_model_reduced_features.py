...
def objective(trial, catboost_loss_functions, eval_metric ,data_splits):
    ...
    params = {
        ...
        "devices": "0,1",
        "thread_count": 96,
        "iterations": trial.suggest_int("iterations", 4000, 4500, step=100),
        "depth": 6,
        "learning_rate": trial.suggest_float("learning_rate", 0.07, 0.1, log=True),
        "l2_leaf_reg": trial.suggest_float("l2_leaf_reg", 10.3, 12.0, log=True),  # Increased range
        "grow_policy": "Depthwise",
        "random_strength": trial.suggest_float("random_strength", 1.44, 2.80, log=True),  # Increased range
        "min_data_in_leaf": trial.suggest_int("min_data_in_leaf", 15, 28),  # Increased range
        "bagging_temperature": trial.suggest_float("bagging_temperature", 0.6, 0.7, step=0.1),
        "border_count": trial.suggest_int("border_count", 200, 240, step=20),
        "od_type": "Iter",
        "random_seed": 42,
        "verbose": 100,
        "early_stopping_rounds": trial.suggest_int("early_stopping_rounds", 150, 180, step=10),
...
def run_optuna(catboost_loss_functions, eval_metric, n_trials=50, data_splits=None):
    ...
    study_name = f"catboost_{catboost_loss_functions}_{eval_metric}_v3"  # "catboost_optuna_study_rmse_min" #"catboost_optuna_study"
...
def train_and_save_model(
    ...
    model_key = f"reduced_{catboost_loss_functions}_{eval_metric}_{timestamp}"
    ...
    model_filename = f"catboost_reduced_{model_key}.cbm"
