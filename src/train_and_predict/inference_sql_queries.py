...
def full_query():
    ...
    queries = {
        ...
        SELECT
            fr.*,
            CASE WHEN hfa.horse_id IS NULL THEN 0 
                ELSE 1 
            END AS has_gps
        FROM future_races fr
        LEFT JOIN horse_form_agg hfa 
                ON fr.horse_id = hfa.horse_id
        ),
        race_check AS (
            SELECT
                course_cd,
        ...
        JOIN race_check rc
            ON fr.course_cd   = rc.course_cd
            AND fr.race_date   = rc.race_date
            AND fr.race_number = rc.race_number
        WHERE fr.course_cd IN (
        'CNL','SAR','PIM','TSA','BEL','MVR','TWO','CLS','KEE','TAM','TTP','TKD', 
        'ELP','PEN','HOU','DMR','TLS','AQU','MTH','TGP','TGG','CBY','LRL', 
        'TED','IND','CTD','ASD','TCD','LAD','TOP'
        )
        AND rc.coverage_count = rc.total_horses
