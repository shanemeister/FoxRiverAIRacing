    ...
    queries = {
        ...
        "runners": """
            SELECT REGEXP_REPLACE(TRIM(UPPER(course_cd)), '\s+$', '') AS course_cd, race_date, race_number, REGEXP_REPLACE(TRIM(UPPER(saddle_cloth_number)), '\s+$', '') AS saddle_cloth_number, 
                ...
                previous_class, net_sentiment, avg_spd_sd, ave_cl_sd,hi_spd_sd, pstyerl, previous_distance, prev_official_fin, race_count, prev_speed_rating, gps_rawspeed, gcsf_speed_figure
            FROM runners
        """,
        "horse": """
