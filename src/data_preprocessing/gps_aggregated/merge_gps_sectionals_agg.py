from pyspark.sql.functions import (
            ...
            min as spark_min, max as spark_max, avg as spark_avg, lag,
            count as spark_count, when, abs, udf, expr, first, last, unix_timestamp)
...
def aggregate_gps_by_section(spark, sectional_results, gpspoint):
    ...
    w = Window.partitionBy(*race_id_cols, "sectionals_gate_name").orderBy("time_stamp")

    gps_accel = gps_filtered \
        .withColumn("prev_speed", lag("speed").over(w)) \
        .withColumn("prev_time", lag("time_stamp").over(w)) \
        .withColumn("time_diff", unix_timestamp("time_stamp") - unix_timestamp("prev_time")) \
        .withColumn("speed_diff", col("speed") - col("prev_speed")) \
        .withColumn("acceleration", col("speed_diff")/col("time_diff"))
    
    accel_stats = gps_accel.groupBy(*race_id_cols, "sectionals_gate_name").agg(
    spark_mean("acceleration").alias("gps_avg_acceleration"),
    spark_max("acceleration").alias("gps_max_acceleration"),
    spark_min("acceleration").alias("gps_min_acceleration"))
    ...
    gps_agg = gps_filtered.groupBy(*race_id_cols, "sectionals_gate_name").agg(
        ...
        spark_max("speed").alias("gps_max_speed"),
        spark_min("speed").alias("gps_min_speed"),
        first("location").alias("gps_first_location"),   # The first GPS point in that section
        last("location").alias("gps_last_location")      # The last GPS point in that section
