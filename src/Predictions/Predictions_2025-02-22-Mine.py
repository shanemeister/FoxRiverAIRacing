...
def write_predictions_to_docx(pred_df, output_docx):
    ...
    for (track_name, course_cd, race_date, race_number), race_df in race_groups:
        ...
        race_has_missing = (race_df['has_gps'].astype(int).sum() > 0) or (race_df['has_gps'].astype(int).sum() > 0)
...
def main():
    ...
    columns_to_select = [
        "course_cd", "horse_id", "group_id", "race_date", "race_number", "track_name", "has_gps", "saddle_cloth_number",
        "horse_name", "recent_avg_speed", "global_speed_score"]
    # Also include the model prediction columns (make sure these names match your table)
    model_columns = ["q_rmse_1", "q_rmse_2", "q_rmse_3", "q_rmse_4", "yeti_top3_1", "yeti_top3_3", "yeti_top4_4"]
    
    columns_to_select.extend([f'"{col}"' for col in model_columns])
    sql_select = ", ".join(columns_to_select)
    sql = f"""
    SELECT {sql_select}
    FROM predictions_2025_02_22_1
