...
def ensemble_predictions(df, model_performance, alpha_blend=0.80):
    """
    Given a DataFrame with model predictions and a 'global_speed_score' column, compute:
      - composite_score: weighted average of model predictions,
      - final_score: a blend of composite_score and global_speed_score,
      - winning_probability: softmax probability (within each race) of the final_score.
    """
    ...
    df['composite_score'] = df[model_cols].apply(lambda row: np.dot(row.values, norm_weights), axis=1)
    # Final blended score
    df['final_score'] = alpha_blend * df['composite_score'] + (1 - alpha_blend) * df['global_speed_score']

    # Calculate winning probabilities using softmax within each race group.
    def race_softmax(s):
        return softmax(s.values)
    df['winning_probability'] = df.groupby(['course_cd', 'race_date', 'race_number'])['final_score'] \
...
def write_predictions_to_docx(pred_df, output_docx):
    ...
    for (track_name, course_cd, race_date, race_number), race_df in race_groups:
        ...
        race_has_missing = (race_df['missing_gps_flag'].astype(int).sum() > 0) or (race_df['race_missing_flag'].astype(int).sum() > 0)
        
        # Sort by saddle_cloth_number using natural sort.
        race_df['saddle_cloth_number'] = race_df['saddle_cloth_number'].apply(natural_sort_key)
        race_df = race_df.sort_values('saddle_cloth_number')
        ...
        for _, row in race_df.iterrows():
            ...
            cells[2].text = f"{row['global_speed_score']:.1f}"
            cells[3].text = f"{row['composite_score']:.2f}"
...
def main():
    ...
    columns_to_select = [
        "course_cd", "race_date", "race_number", "track_name", "saddle_cloth_number",
        "horse_name", "global_speed_score", "missing_gps_flag", "race_missing_flag", "has_gps"
    ]
    # Also include the model prediction columns (make sure these names match your table)
    model_columns = [
        "YetiRank_top_4_NDCG_top_4", "YetiRank_top_1_NDCG_top_1", "QueryRMSE_NDCG_top_2",
        "YetiRank_top_2_NDCG_top_4", "YetiRank_top_1_NDCG_top_3", "YetiRank_top_2_NDCG_top_3",
        "YetiRank_top_2_NDCG_top_2", "QueryRMSE_NDCG_top_4", "QueryRMSE_NDCG_top_3",
        "YetiRank_top_4_NDCG_top_3"
    ]
    columns_to_select.extend([f'"{col}"' for col in model_columns])
    ...
    output_docx = os.path.join(script_dir, f"final_ensemble_predictions_CoPilot_{current_date}.docx")
